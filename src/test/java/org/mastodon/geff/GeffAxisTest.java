...
public class GeffAxisTest {
    ...
    void testConstructorWithRequiredFields() {
        GeffAxis axis = new GeffAxis("x", GeffAxis.TYPE_SPACE, GeffAxis.UNIT_MICROMETERS);

        assertEquals("x", axis.getName());
        assertEquals(GeffAxis.TYPE_SPACE, axis.getType());
        assertEquals(GeffAxis.UNIT_MICROMETERS, axis.getUnit());
        assertNull(axis.getMin());
        assertNull(axis.getMax());
        assertFalse(axis.hasBounds());
    }
    ...
    void testConstructorWithAllFields() {
        GeffAxis axis = new GeffAxis("t", GeffAxis.TYPE_TIME, GeffAxis.UNIT_SECONDS, 0.0, 125.0);

        assertEquals("t", axis.getName());
        assertEquals(GeffAxis.TYPE_TIME, axis.getType());
        assertEquals(GeffAxis.UNIT_SECONDS, axis.getUnit());
    ...
    void testTimeAxisFactoryMethods() {
        // With bounds
        GeffAxis timeAxisWithBounds = GeffAxis.createTimeAxis("t", GeffAxis.UNIT_SECONDS, 0.0, 125.0);
        assertEquals("t", timeAxisWithBounds.getName());
        assertEquals(GeffAxis.TYPE_TIME, timeAxisWithBounds.getType());
        assertEquals(GeffAxis.UNIT_SECONDS, timeAxisWithBounds.getUnit());
        assertTrue(timeAxisWithBounds.hasBounds());

        // Without bounds
        GeffAxis timeAxisNoBounds = GeffAxis.createTimeAxis("t", GeffAxis.UNIT_SECONDS);
        assertEquals("t", timeAxisNoBounds.getName());
        assertEquals(GeffAxis.TYPE_TIME, timeAxisNoBounds.getType());
        assertEquals(GeffAxis.UNIT_SECONDS, timeAxisNoBounds.getUnit());
    ...
    void testSpaceAxisFactoryMethods() {
        // With bounds
        GeffAxis spaceAxisWithBounds = GeffAxis.createSpaceAxis("x", GeffAxis.UNIT_MICROMETERS, 764.42, 2152.3);
        assertEquals("x", spaceAxisWithBounds.getName());
        assertEquals(GeffAxis.TYPE_SPACE, spaceAxisWithBounds.getType());
        assertEquals(GeffAxis.UNIT_MICROMETERS, spaceAxisWithBounds.getUnit());
        assertTrue(spaceAxisWithBounds.hasBounds());
        assertEquals(2152.3 - 764.42, spaceAxisWithBounds.getRange(), 0.001);

        // Without bounds
        GeffAxis spaceAxisNoBounds = GeffAxis.createSpaceAxis("y", GeffAxis.UNIT_MICROMETERS);
        assertEquals("y", spaceAxisNoBounds.getName());
        assertEquals(GeffAxis.TYPE_SPACE, spaceAxisNoBounds.getType());
        assertEquals(GeffAxis.UNIT_MICROMETERS, spaceAxisNoBounds.getUnit());
    ...
    void testTypeValidation() {
        ...
        assertDoesNotThrow(() -> axis.setType(null)); // null should be allowed for setter
    ...
    void testAxisValidation() {
        // Valid axis
        GeffAxis validAxis = new GeffAxis("x", GeffAxis.TYPE_SPACE, GeffAxis.UNIT_MICROMETERS, 0.0, 100.0);
        assertDoesNotThrow(() -> validAxis.validate());

        // Missing name
        GeffAxis missingName = new GeffAxis(null, GeffAxis.TYPE_SPACE, GeffAxis.UNIT_MICROMETERS);
        assertThrows(IllegalArgumentException.class, () -> missingName.validate());

        // Empty name
        GeffAxis emptyName = new GeffAxis("", GeffAxis.TYPE_SPACE, GeffAxis.UNIT_MICROMETERS);
        assertThrows(IllegalArgumentException.class, () -> emptyName.validate());

        // Missing type
        GeffAxis missingType = new GeffAxis("x", null, GeffAxis.UNIT_MICROMETERS);
        assertThrows(IllegalArgumentException.class, () -> missingType.validate());

        // Invalid type - create axis with valid type then test validation separately
        GeffAxis validAxisForTypeTest = new GeffAxis("x", GeffAxis.TYPE_SPACE, GeffAxis.UNIT_MICROMETERS);
        assertDoesNotThrow(() -> validAxisForTypeTest.validate()); // This should pass

        // Test that setType rejects invalid types
        GeffAxis axisForInvalidType = new GeffAxis();
        axisForInvalidType.setName("x");
        axisForInvalidType.setUnit(GeffAxis.UNIT_MICROMETERS);
    ...
    void testEqualsAndHashCode() {
        GeffAxis axis1 = new GeffAxis("x", GeffAxis.TYPE_SPACE, GeffAxis.UNIT_MICROMETERS, 0.0, 100.0);
        GeffAxis axis2 = new GeffAxis("x", GeffAxis.TYPE_SPACE, GeffAxis.UNIT_MICROMETERS, 0.0, 100.0);
        GeffAxis axis3 = new GeffAxis("y", GeffAxis.TYPE_SPACE, GeffAxis.UNIT_MICROMETERS, 0.0, 100.0);
        ...
        // Note: axis1 and axis3 may or may not have the same hashCode (hash collision
        // allowed)
    }
    ...
    void testToString() {
        GeffAxis axisWithBounds = new GeffAxis("t", GeffAxis.TYPE_TIME, GeffAxis.UNIT_SECONDS, 0.0, 125.0);
        ...
        GeffAxis axisNoBounds = new GeffAxis("x", GeffAxis.TYPE_SPACE, GeffAxis.UNIT_MICROMETERS);
    ...
    void testSpecificationExamples() {
        // Time axis: {'name': 't', 'type': "time", 'unit': "seconds", 'min': 0, 'max':
        // 125}
        GeffAxis timeAxis = new GeffAxis("t", GeffAxis.TYPE_TIME, GeffAxis.UNIT_SECONDS, 0.0, 125.0);
        assertDoesNotThrow(() -> timeAxis.validate());
        assertEquals("t", timeAxis.getName());
        assertEquals(GeffAxis.TYPE_TIME, timeAxis.getType());
        assertEquals(GeffAxis.UNIT_SECONDS, timeAxis.getUnit());
        assertEquals(0.0, timeAxis.getMin());
        assertEquals(125.0, timeAxis.getMax());

        // Space axis: {'name': 'z', 'type': "space", 'unit': "micrometers", 'min':
        // 1523.36, 'max': 4398.1}
        GeffAxis zAxis = new GeffAxis("z", GeffAxis.TYPE_SPACE, GeffAxis.UNIT_MICROMETERS, 1523.36, 4398.1);
        assertDoesNotThrow(() -> zAxis.validate());
        assertEquals("z", zAxis.getName());
        assertEquals(GeffAxis.TYPE_SPACE, zAxis.getType());
        assertEquals(GeffAxis.UNIT_MICROMETERS, zAxis.getUnit());
        assertEquals(1523.36, zAxis.getMin());
        assertEquals(4398.1, zAxis.getMax());

        // Space axis: {'name': 'y', 'type': "space", 'unit': "micrometers", 'min':
        // 81.667, 'max': 1877.7}
        GeffAxis yAxis = new GeffAxis("y", GeffAxis.TYPE_SPACE, GeffAxis.UNIT_MICROMETERS, 81.667, 1877.7);
        assertDoesNotThrow(() -> yAxis.validate());

        // Space axis: {'name': 'x', 'type': "space", 'unit': "micrometers", 'min':
        // 764.42, 'max': 2152.3}
        GeffAxis xAxis = new GeffAxis("x", GeffAxis.TYPE_SPACE, GeffAxis.UNIT_MICROMETERS, 764.42, 2152.3);
