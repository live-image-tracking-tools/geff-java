...
public class GeffTest {
    ...
    void setUp() {
        ...
        for (int i = 0; i < 4; i++) {
            GeffEdge edge = new GeffEdge(i, i, i + 1, 0.5, 10.0); // id, source, target, score, distance
            testEdges.add(edge);
        }
    }
    ...
    void testGeffObjectCreation() {
        ...
        assertDoesNotThrow(() -> {
            // Test that we can create the components that would go into a Geff object
    ...
    void testNodeDataStructure() {
        ...
        // Note: getPosition is deprecated, testing individual coordinates instead
    }
    ...
    void testMetadataValidation() {
        ...
        GeffAxis[] invalidAxes = {
                new GeffAxis("x", GeffAxis.TYPE_SPACE, GeffAxis.UNIT_MICROMETERS, 100.0, 50.0) // min > max
        };
    ...
    void testEdgeListOperations() {
        ...
        GeffEdge newEdge = new GeffEdge(10, 100, 101, 0.8, 15.0); // id, source, target, score, distance
